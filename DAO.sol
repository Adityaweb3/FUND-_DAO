...
// 1. Users can provide up to 4 MATIC tokens to become a Member of the DAO and 
//can provide more than 4 tokens to become a Stakeholder in the DAO.

//2. A Member can create new proposals and request funding.
//Every new proposal requires the Member to add 3 MATIC tokens to the DAO treasury.

//3. If the proposal is accepted after the voting period the initial 3 MATIC tokens are refunded,
// otherwise the tokens are reserved in the DAO treasury.

// 4. When a Member creates a new proposal 
// Stakeholders have 3 days to vote on the proposal, either in favor or against it.

// 5. At the end of the voting period, if the proposal is selected, 
//Stakeholders have an option to provide the requested funds to the Member who made the proposal.
...
// We are using AccesControl to manage the member and stakeholder roles in a DAO 

import "@openzeppelin/contracts/security/ReentrancyGuard.sol";
// We are Using ReentrancyGuard to make sure there can't be any nested reentrant call 
...
contract FundingDAO is ReentrancyGuard, AccessControl {
    ...
    struct Funding {
        //The Funding struct holds the data related to payments into the DAO 
        address payer;
        uint256 amount;
        uint256 timestamp;
    }

    mapping(uint256 => Proposal) private proposals; // proposals stores the instances of Proposal structs, mapped to a unique number.
    mapping(address => uint256) private stakeholders; //stakeholders maps a Stakeholder's address to their balance of tokens provided to DAO treasury.
    mapping(address => uint256) private members; // members maps a Member's address to their balance of tokens provided to the DAO treasury
    mapping(address => uint256[]) private votes; // votes contains the array of proposal IDs for each Stakeholder to keep track of their votes.
